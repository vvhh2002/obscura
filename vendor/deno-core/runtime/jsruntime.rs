...
use super::jsrealm::ManagedJsRealm;
use super::jsrealm::ManagedJsRealmInner;
...
pub(crate) struct InnerIsolateState {
    ...
    // Keep registered managed realms alive until explicit retirement or isolate
    // cleanup. Browser owners sever a stable WindowProxy backend before calling
    // ManagedJsRealm::retire, so the context cannot be re-entered after its
    // embedder slots are reclaimed. The shared registry lets that handle remove
    // itself without borrowing JsRuntime through a FrameRealm drop path.
    managed_realms: Rc<RefCell<Vec<Rc<ManagedJsRealmInner>>>>,
...
impl InnerIsolateState {
    ...
    pub fn cleanup(&mut self) {
        ...
        for realm in self.managed_realms.borrow_mut().drain(..) {
            realm.destroy();
        }
...
pub(crate) static BUILTIN_SOURCES: [InternalSourceFile; 1] = [internal_source_file!("01_core.js")];

/// Executed after `BUILTIN_SOURCES` are executed. Provides a thin ES module
/// that exports `core`, `internals` and `primordials` objects.
pub(crate) static BUILTIN_ES_MODULES: [ExtensionFileSource; 1] = [ExtensionFileSource::new(
    "ext:core/mod.js",
...
pub struct JsRuntimeState {
    ...
    pub(crate) eval_context_get_code_cache_cb: RefCell<Option<EvalContextGetCodeCacheCb>>,
    pub(crate) eval_context_code_cache_ready_cb: RefCell<Option<EvalContextCodeCacheReadyCb>>,
...
impl JsRuntime {
    ...
    fn new_inner(mut options: RuntimeOptions, will_snapshot: bool) -> Result<JsRuntime, CoreError> {
        let init_mode = InitMode::from_options(&options);
        let mut extensions = std::mem::take(&mut options.extensions);
        let mut isolate_allocations = IsolateAllocations::default();

        let enable_stack_trace_in_ops = options.maybe_op_stack_trace_callback.is_some();
        ...
        let lazy_extensions = extension_set::setup_op_state(&mut op_state, &mut extensions);
        ...
        let mut sources = extension_set::into_sources_and_source_maps(
            ...
            |source| mark_as_loaded_from_fs_during_snapshot(&mut files_loaded, &source.code),
        )?;
        ...
        let (eval_context_get_code_cache_cb, eval_context_set_code_cache_cb) = options
        ...
        let state_rc = Rc::new(JsRuntimeState {
            ...
            eval_context_get_code_cache_cb: RefCell::new(eval_context_get_code_cache_cb),
            eval_context_code_cache_ready_cb: RefCell::new(eval_context_set_code_cache_cb),
            waker,
        ...
        let (global_template_middleware, global_object_middlewares, additional_references) =
        ...
        if let Some((snapshotted_data, mut data_store)) = snapshotted_data {
            *exception_state.js_handled_promise_rejection_cb.borrow_mut() = snapshotted_data
        ...
        if context_state.ext_import_meta_proto.borrow().is_none() {
            ...
            *context_state.ext_import_meta_proto.borrow_mut() = Some(v8::Global::new(scope, obj));
        }
        ...
        let mut js_runtime = JsRuntime {
            inner: InnerIsolateState {
                ...
                managed_realms: Rc::new(RefCell::new(Vec::new())),
        ...
        // internal JS and then execute code provided by extensions...
        {
            ...
            // ) {
            if init_mode == InitMode::New {
                js_runtime.execute_virtual_ops_module(context_global, module_map.clone());
            }

            if init_mode == InitMode::New {
                js_runtime.execute_builtin_sources(&realm, &module_map, &mut files_loaded)?;
            }
    ...
    pub fn lazy_init_extensions(&self, ext_args: Vec<ExtensionArguments>) -> Result<(), CoreError> {
    ...
    /// Restore a secondary context from the runtime's startup snapshot and
    /// attach an independent module map to it.
    ///
    /// A context returned by `v8::Context::from_snapshot` is not on its own a
    /// deno_core realm. In particular, dynamic import reads the module map from
    /// a context embedder slot. This constructor initializes the required slots
    /// and registers the realm so [`JsRuntime::poll_event_loop`] advances its
    /// module graph alongside the main realm.
    pub fn create_managed_realm_from_snapshot(
        &mut self,
        snapshot_index: usize,
        options: CreateRealmOptions,
    ) -> Option<ManagedJsRealm> {
        ...
        let context = {
            ...
            let context =
                v8::Context::from_snapshot(scope, snapshot_index, v8::ContextOptions::default())?;
            unsafe {
                context.set_aligned_pointer_in_embedder_data(
                    super::jsrealm::CONTEXT_STATE_SLOT_INDEX,
                    // The context slot owns one strong count, exactly like the main
                    // realm. ManagedJsRealmInner::destroy reclaims it with from_raw.
                    Rc::into_raw(context_state.clone()) as *mut c_void,
                );
                context.set_aligned_pointer_in_embedder_data(
                    super::jsrealm::MODULE_MAP_SLOT_INDEX,
        ...
        let inner = Rc::new(ManagedJsRealmInner::new(
            context_state,
            context,
            module_map,
            function_templates,
        ));
        self.inner.managed_realms.borrow_mut().push(inner.clone());
        Some(ManagedJsRealm(
            inner,
            Rc::downgrade(&self.inner.managed_realms),
        ))
    }

    /// Number of secondary realms still registered for event-loop polling.
    pub fn managed_realm_count(&self) -> usize {
        self.inner.managed_realms.borrow().len()
    }
    ...
    async fn init_extension_js_inner(
        ...
        let ext_loader = Rc::new(ExtModuleLoader::new(sources, ext_code_cache.clone()));
        *module_map.loader.borrow_mut() = ext_loader.clone();
        ...
        for specifier in loaded_sources.esm_entry_points {
            let Some(mod_id) = module_map.get_id(&specifier, RequestedModuleType::None) else {
    ...
    pub fn eval<'s, T>(scope: &mut v8::HandleScope<'s>, code: &str) -> Option<v8::Local<'s, T>>
    where
    ...
    fn store_js_callbacks(&mut self, realm: &JsRealm, will_snapshot: bool) {
        let (event_loop_tick_cb, build_custom_error_cb, wasm_instance_fn) = {
            ...
            // look up them again?
            let deno_obj: v8::Local<v8::Object> = bindings::get(scope, global, DENO, "Deno");
            let core_obj: v8::Local<v8::Object> = bindings::get(scope, deno_obj, CORE, "Deno.core");

            let event_loop_tick_cb: v8::Local<v8::Function> =
                bindings::get(scope, core_obj, EVENT_LOOP_TICK, "Deno.core.eventLoopTick");
    ...
    pub fn scoped_call_with_args(
        ...
        if promise.is_none() {
            ...
            return RcPromiseFuture::new(exception_to_err_result(scope, exception, false, true));
        }
    ...
    pub async fn call_and_await(
        ...
        self.with_event_loop_promise(call, PollEventLoopOptions::default())
            .await
    }
    ...
    fn pump_v8_message_loop(&self, scope: &mut v8::HandleScope) -> Result<(), CoreError> {
        ...
        match tc_scope.exception() {
            None => Ok(()),
            Some(exception) => exception_to_err_result(tc_scope, exception, false, true),
        }
    }
    ...
    pub fn maybe_init_inspector(&mut self) {
        ...
        let scope =
            &mut v8::HandleScope::with_context(self.inner.v8_isolate.as_mut(), context.clone());
    ...
    pub async fn resolve_value(
        ...
        self.with_event_loop_promise(resolve, PollEventLoopOptions::default())
            .await
    }
    ...
    pub fn poll_event_loop(
        ...
        let managed_realms = self.inner.managed_realms.borrow().clone();
        let mut managed_pending = false;
        for realm in managed_realms {
            if realm.is_destroyed() {
                continue;
            }
            realm.record_event_loop_poll();
            let managed_result = {
                ...
                let context = v8::Local::new(&mut isolate_scope, realm.context());
                let mut scope = v8::ContextScope::new(&mut isolate_scope, context);
                self.poll_managed_realm(cx, &mut scope, &realm)
            };
            match managed_result {
                Poll::Ready(Ok(())) => {}
                Poll::Ready(Err(error)) => return Poll::Ready(Err(error)),
                Poll::Pending => managed_pending = true,
            }
        }

        // SAFETY: We know this isolate is valid and non-null at this time
        let mut isolate_scope = v8::HandleScope::new(unsafe { &mut *self.v8_isolate_ptr() });
        let context = v8::Local::new(&mut isolate_scope, self.inner.main_realm.context());
        let mut scope = v8::ContextScope::new(&mut isolate_scope, context);
        match self.poll_event_loop_inner(cx, &mut scope, poll_options) {
            Poll::Ready(Ok(())) if managed_pending => Poll::Pending,
            result => result,
        }
    }

    fn poll_managed_realm(
        &self,
        cx: &mut Context,
        scope: &mut v8::HandleScope,
        realm: &ManagedJsRealmInner,
    ) -> Poll<Result<(), CoreError>> {
        self.inner.state.waker.register(cx.waker());
        let modules = &realm.module_map;
        ...
        let has_pending_dyn_module_evaluation = modules.has_pending_dyn_module_evaluation();
        ...
        let context_state = JsRealm::state_from_scope(scope);
        let pending_state = EventLoopPendingState::new(scope, &context_state, modules);
        ...
        if has_pending_module_evaluation
            ...
            && !pending_state.has_tick_scheduled
        {
            return Poll::Ready(Err(find_and_report_stalled_level_await(scope, modules)));
        }
        ...
        if has_pending_dyn_module_evaluation
            ...
            && !pending_state.has_tick_scheduled
        {
            let idle = &modules.dyn_module_evaluate_idle_counter;
            if idle.get() > 1 {
                return Poll::Ready(Err(find_and_report_stalled_level_await(scope, modules)));
            }
            idle.set(idle.get() + 1);
    ...
    fn poll_event_loop_inner(
        ...
        // first).
        let dispatched_ops =
            Self::do_js_event_loop_tick_realm(cx, scope, context_state, exception_state)?;
        ...
        if pending_state.has_pending_module_evaluation {
                ...
                || pending_state.has_tick_scheduled
            {
                ...
                return Poll::Ready(Err(find_and_report_stalled_level_await_in_any_realm(
                    scope, &realm.0,
                )));
            }
        }
...
fn find_and_report_stalled_level_await_in_any_realm(
    ...
    find_and_report_stalled_level_await(scope, &inner_realm.module_map())
}

fn find_and_report_stalled_level_await(
    scope: &mut v8::HandleScope,
    module_map: &Rc<ModuleMap>,
) -> CoreError {
...
fn create_context<'a>(
    ...
    let context = if has_snapshot {
        ...
        v8::Context::from_snapshot(scope, 1, Default::default())
            .unwrap_or_else(|| v8::Context::from_snapshot(scope, 0, Default::default()).unwrap())
    } else {
...
impl JsRuntimeForSnapshot {
    ...
    pub fn try_new(mut options: RuntimeOptions) -> Result<JsRuntimeForSnapshot, CoreError> {
        setup::init_v8(
            options.v8_platform.take(),
            true,
            options.unsafe_expose_natives_and_gc(),
    ...
    pub fn snapshot(mut self) -> Box<[u8]> {
        ...
        let original_sources = std::mem::take(&mut self.0.allocations.original_sources);
...
impl EventLoopPendingState {
    ...
    pub fn new(scope: &mut v8::HandleScope<()>, state: &ContextState, modules: &ModuleMap) -> Self {
        ...
        let has_pending_refed_ops =
            has_pending_tasks || has_pending_refed_timers || num_pending_ops > num_unrefed_ops;
        EventLoopPendingState {
            has_pending_ops: has_pending_refed_ops || has_pending_timers || (num_pending_ops > 0),
...
impl JsRuntime {
    #[cfg(test)]
    pub(crate) fn instantiate_module(&mut self, id: ModuleId) -> Result<(), v8::Global<v8::Value>> {
        let isolate = &mut self.inner.v8_isolate;
        let realm = JsRealm::clone(&self.inner.main_realm);
        let scope = &mut realm.handle_scope(isolate);
    ...
    /// Return every module id and canonical specifier in an instantiated static
    /// graph. Browser schedulers use this to make their execution cache match
    /// V8's graph-wide evaluation semantics.
    pub fn module_graph_specifiers(
        &self,
        module_id: ModuleId,
    ) -> Result<Vec<(ModuleId, String)>, CoreError> {
        self.inner
            .main_realm
            .0
            .module_map
            .graph_specifiers(module_id)
    }
    ...
    pub async fn load_main_es_module_from_code(
        ...
        self.inner
            .main_realm
            .load_main_es_module_from_code(isolate, specifier, Some(code.into_module_code()))
            .await
    }
    ...
    pub async fn load_side_es_module_from_code(
        ...
        self.inner
            .main_realm
            .load_side_es_module_from_code(isolate, specifier, Some(code.into_module_code()))
            .await
    }

    /// Compile an explicit-code side-module root without making its canonical
    /// URL importable by name. Intended for distinct HTML inline module scripts
    /// that share a document URL.
    pub async fn load_unregistered_side_es_module_from_code(
        &mut self,
        specifier: &ModuleSpecifier,
        code: impl IntoModuleCodeString,
    ) -> Result<ModuleId, CoreError> {
        ...
        self.inner
            .main_realm
            .load_unregistered_side_es_module_from_code(isolate, specifier, code.into_module_code())
            .await
    }
    ...
    fn do_js_event_loop_tick_realm(
        ...
        let mut retries = 3;
        while let Poll::Ready(tasks) = context_state.task_spawner_factory.poll_inner(cx) {
        ...
        let mut args: SmallVec<[v8::Local<v8::Value>; 32]> = SmallVec::with_capacity(32);
        ...
        loop {
            ...
            let Poll::Ready((promise_id, op_id, res)) = context_state.pending_ops.poll_ready(cx)
            else {
                break;
            };
            ...
            .is_empty()
        {
            ...
            let mut pending_rejections = exception_state.pending_promise_rejections.borrow_mut();
        ...
        let js_event_loop_tick_cb = js_event_loop_tick_cb.as_ref().unwrap().open(tc_scope);
