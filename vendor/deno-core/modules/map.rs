...
type CodeCacheReadyCallback = Box<dyn FnOnce(&[u8]) -> Pin<Box<dyn Future<Output = ()>>>>;
...
pub(crate) struct ModuleMap {
    ...
    preparing_dynamic_imports: RefCell<FuturesUnordered<Pin<Box<PrepareLoadFuture>>>>,
    preparing_dynamic_imports_pending: Cell<bool>,
    pending_dynamic_imports: RefCell<FuturesUnordered<StreamFuture<RecursiveModuleLoad>>>,
    ...
    code_cache_ready_futs: RefCell<FuturesUnordered<Pin<Box<CodeCacheReadyFuture>>>>,
...
impl ModuleMap {
    ...
    pub(crate) fn get_type_by_module(&self, global: &v8::Global<v8::Module>) -> Option<ModuleType> {
    ...
    pub(crate) fn new_module(
        ...
        let module_url_found = if let Some(module_url_found) = module_url_found {
            let (module_url_found1, module_url_found2) = module_url_found.into_cheap_copy();
        ...
        let module_id = match module_type {
            ModuleType::JavaScript => {
                ...
                let (code_cache_info, module_url_found) = if let Some(code_cache) = code_cache {
                    let (module_url_found1, module_url_found2) = module_url_found.into_cheap_copy();
            ...
            ModuleType::Other(module_type) => {
                ...
                let custom_module_evaluation_cb = state.custom_module_evaluation_cb.as_ref();

                let Some(custom_evaluation_cb) = custom_module_evaluation_cb else {
                    return Err(ModuleError::Concrete(ModuleConcreteError::UnsupportedKind(
                ...
                let module_evaluation_kind =
                    custom_evaluation_cb(scope, module_type.clone(), &module_url_found, code)
                ...
                match module_evaluation_kind {
                    ...
                    CustomModuleEvaluationKind::ComputedAndSynthetic(
                        ...
                        let _synthetic_mod_id =
                            self.new_synthetic_module(scope, url1, synthetic_module_type, exports);
    ...
    pub fn new_synthetic_module(
        ...
        let id =
            self.data
                .borrow_mut()
                .create_module_info(name, module_type, handle, false, vec![]);
    ...
    /// Compile an explicit-code side-module root under its canonical source URL
    /// while keeping it out of the URL lookup table. The caller supplies its
    /// ModuleId directly to RecursiveModuleLoad, allowing multiple inline roots
    /// from one document without changing ScriptOrigin/import.meta.url.
    pub(crate) fn new_unregistered_es_module(
        &self,
        scope: &mut v8::HandleScope,
        name: impl IntoModuleName,
        source: impl IntoModuleCodeString,
    ...
    pub(crate) fn new_module_from_js_source(
        ...
        if try_store_code_cache && !self.will_snapshot {
            if let Some(code_cache_info) = code_cache_info.take() {
                ...
                let code_cache = unbound_module_script.create_code_cache().ok_or_else(|| {
                    ModuleError::Concrete(ModuleConcreteError::UnboundModuleScriptCodeCache)
                })?;
                let fut = async move { (code_cache_info.ready_callback)(&code_cache).await }
                    .boxed_local();
        ...
        for i in 0..module_requests.length() {
            ...
            // is thrown here
            {
                ...
                if let Some(validate_import_attributes_cb) = &state.validate_import_attributes_cb {
            ...
            let requested_module_type = get_requested_module_type_from_attributes(&attributes);
        ...
        let id = if register_by_name {
            self.data
                .borrow_mut()
                .create_module_info(name, module_type, handle, main, requests)
        } else {
            self.data.borrow_mut().create_unregistered_module_info(
                name,
                module_type,
                handle,
                main,
    ...
    pub(crate) fn new_wasm_module(
        ...
        let wasm_module_analysis =
            WasmDeps::parse(bytes, wasm_dep_analyzer::ParseOptions { skip_types: true })
        ...
        let js_wasm_module_source = render_js_wasm_module(name.as_str(), wasm_module_analysis);

        let synthetic_module_type = ModuleType::Other("$$deno-core-internal-wasm-module".into());
    ...
    pub(crate) fn new_json_module(
        ...
        let source_str =
            v8::String::new_from_utf8(scope, strip_bom(code.as_bytes()), v8::NewStringType::Normal)
                .unwrap();
        let tc_scope = &mut v8::TryCatch::new(scope);
    ...
    fn module_resolve_callback<'s>(
      ...
      unsafe { scope.get_slot::<*const Self>().unwrap().as_ref().unwrap() };
        ...
        let assertions =
            parse_import_attributes(scope, import_attributes, ImportAttributesKind::StaticImport);
        let maybe_module =
    ...
    fn resolve_callback<'s>(
        ...
        let module_type = get_requested_module_type_from_attributes(&import_attributes);
    ...
    pub(crate) fn get_requested_modules(&self, id: ModuleId) -> Option<Vec<ModuleRequest>> {
    ...
    /// Return the complete instantiated static graph rooted at `root_id`.
    ///
    /// Embedders which schedule several browser module-script elements need to
    /// remember that evaluating a root also evaluates every static descendant.
    /// Keeping this traversal inside ModuleMap avoids reconstructing graph
    /// membership from timing-sensitive ModuleLoader callbacks.
    pub(crate) fn graph_specifiers(
        &self,
        root_id: ModuleId,
    ) -> Result<Vec<(ModuleId, String)>, CoreError> {
        let mut pending = std::collections::VecDeque::from([root_id]);
        ...
        while let Some(module_id) = pending.pop_front() {
            ...
            let name = self
                ...
                .ok_or_else(|| CoreError::MissingFromModuleMap(module_id.to_string()))?;
            graph.push((module_id, name));
            let requests = self
                .get_requested_modules(module_id)
                .ok_or_else(|| CoreError::MissingFromModuleMap(module_id.to_string()))?;
            for request in requests {
                let child = self
                    .get_id(request.specifier.as_str(), &request.requested_module_type)
                    .ok_or_else(|| {
                        CoreError::MissingFromModuleMap(request.specifier.to_string())
                    })?;
    ...
    pub(crate) async fn load_main(
        ...
        let load = RecursiveModuleLoad::main(specifier.as_ref(), module_map_rc.clone());
        load.prepare().await?;
        Ok(load)
    }
    ...
    pub(crate) async fn load_side(
        ...
        let load = RecursiveModuleLoad::side(specifier.as_ref(), module_map_rc.clone());
        load.prepare().await?;
        Ok(load)
    }
    ...
    pub(crate) fn load_dynamic_import(
        ...
        let resolve_result = self.resolve(specifier, referrer, ResolutionKind::DynamicImport);
        ...
        let fut = match resolve_result {
            Ok(_) => async move { (load.id, load.prepare().await.map(|()| load)) }.boxed_local(),
    ...
    pub(crate) fn has_pending_dynamic_imports(&self) -> bool {
        self.preparing_dynamic_imports_pending.get() || self.pending_dynamic_imports_pending.get()
    }
    ...
    pub fn mod_evaluate(
        ...
        let receiver =
            receiver.map(|res| res.unwrap_or_else(|_| Err(CoreError::ExecutionTerminated)));
        let mut status = module.get_status();
        match status {
            // Browser module maps evaluate a module at most once. A second script
            // element naming a graph which already ran is a no-op, not a Rust panic
            // across V8's FFI boundary.
            v8::ModuleStatus::Evaluated if !self.pending_mod_evaluation.get() => {
                _ = sender.send(Ok(()));
                return receiver;
            }
            v8::ModuleStatus::Evaluated => {
                _ = sender.send(Err(deno_error::JsErrorBox::generic(
                    "module evaluation is already in progress",
                )
                .into()));
                return receiver;
            }
            v8::ModuleStatus::Errored => {
                ...
                _ = sender.send(Err(CoreError::Js(JsError::from_v8_exception(
                    tc_scope, exception,
                ))));
                return receiver;
            }
            ...
            _ => {
                let name = self.get_name_by_id(id).unwrap_or_else(|| id.to_string());
                _ = sender.send(Err(deno_error::JsErrorBox::generic(format!(
                    "module is not instantiated: {name} ({status:?})"
                ))
                .into()));
                return receiver;
            }
        ...
        let Some(value) = module.evaluate(tc_scope) else {
            if tc_scope.has_terminated() || tc_scope.is_execution_terminating() {
                ...
                _ = sender.send(exception_to_err_result(tc_scope, undefined, true, false));
            } else {
                ...
                match promise.state() {
                    PromiseState::Fulfilled => {
                        if let Some(exception) = tc_scope.exception() {
                            _ =
                                sender.sender.take().unwrap().send(exception_to_err_result(
    ...
    pub(crate) fn mod_evaluate_sync(
        ...
        let status = module.get_status();
        debug_assert!(status == v8::ModuleStatus::Evaluated || status == v8::ModuleStatus::Errored);
        let promise = v8::Local::<v8::Promise>::try_from(value)
            .expect("Expected to get promise as module evaluation result");
    ...
    fn dynamic_import_module_evaluate(
        ...
        if let Some(value) = maybe_value {
            ...
            self.pending_dyn_mod_evaluations
                .borrow_mut()
    ...
    fn evaluate_dyn_imports(&self, scope: &mut v8::HandleScope) -> bool {
        ...
        let pending = std::mem::take(self.pending_dyn_mod_evaluations.borrow_mut().deref_mut());
        ...
        self.pending_dyn_mod_evaluations_pending
    ...
    fn poll_prepare_dyn_imports(
        ...
        loop {
            ...
            self.preparing_dynamic_imports_pending
    ...
    fn poll_dyn_imports(
        ...
        loop {
            ...
            if let Poll::Ready(Some(load_stream_poll)) = poll_result {
                ...
                match maybe_result {
                    ...
                    _ => {
                        ...
                        self.dynamic_import_module_evaluate(scope, dyn_import_id, module_id)?;
                    }
                }
            ...
            // There are no active dynamic import loads, or none are ready.
            self.pending_dynamic_imports_pending
                .set(!self.pending_dynamic_imports.borrow().is_empty());
            return Poll::Ready(Ok(()));
        }
    }

    fn poll_code_cache_ready(&self, cx: &mut Context) -> Poll<Result<(), CoreError>> {
        if !self.pending_code_cache_ready.get() {
            return Poll::Ready(Ok(()));
        }

        loop {
            let poll_result = self.code_cache_ready_futs.borrow_mut().poll_next_unpin(cx);

            if let Poll::Ready(Some(_)) = poll_result {
                continue;
            }

            self.pending_code_cache_ready
    ...
    pub(crate) fn find_stalled_top_level_await(
        ...
        if let Some(root_module_id) = root_module_id {
            let messages =
                self.get_stalled_top_level_await_message_for_module(scope, root_module_id);
        ...
        for module_id in 0..self.data.borrow().handles.len() {
            let messages = self.get_stalled_top_level_await_message_for_module(scope, module_id);
            if !messages.is_empty() {
                return messages;
            }
        }
    ...
    pub(crate) fn lazy_load_es_module_with_code(
        ...
        if !result.is_undefined() {
            return Err(exception_to_err_result::<()>(scope, result, false, true).unwrap_err());
        }
    ...
    pub(crate) fn add_lazy_loaded_esm_source(&self, specifier: ModuleName, code: ModuleCodeString) {
    ...
    pub(crate) fn lazy_load_esm_module(
        ...
        // Check if this module has already been loaded.
        {
            ...
            if let Some(id) = module_map_data.get_id(module_specifier, RequestedModuleType::None) {
        ...
        let load_response = loader.load(&specifier, None, false, RequestedModuleType::None);
...
fn render_js_wasm_module(specifier: &str, wasm_deps: WasmDeps) -> String {
    ...
    StringBuilder::build(|builder| {
        ...
        if !aggregated_imports.is_empty() {
            ...
            builder.append("const modInstance = new import.meta.WasmInstance(wasmMod);\n");
        }
    ...
    let deps = WasmDeps {
        imports: vec![
            wasm_dep_analyzer::Import {
                name: "foo",
                module: "./import.js",
                import_type: wasm_dep_analyzer::ImportType::Tag(wasm_dep_analyzer::TagType {
                    kind: 1,
                    type_index: 1,
                }),
            },
