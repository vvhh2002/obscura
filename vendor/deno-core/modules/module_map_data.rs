...
impl<T> ModuleNameTypeMap<T> {
    ...
    pub fn insert(&mut self, module_type: &RequestedModuleType, name: FastString, module: T) {
    ...
    pub fn drain(mut self, mut f: impl FnMut(usize, &RequestedModuleType, ModuleName, T)) {
        let mut i = 0;
...
pub(crate) type SyntheticModuleExports = Vec<(v8::Global<v8::String>, v8::Global<v8::Value>)>;
...
impl ModuleMapData {
    pub fn create_module_info(
        ...
        self.create_module_info_inner(name, module_type, handle, main, requests, true)
    }

    /// Retain a module by id and handle without making its canonical source URL
    /// an importable name. Explicit-code side-module roots use this path so two
    /// inline scripts can share the document URL while remaining distinct V8
    /// modules. Their dependencies are still registered normally.
    pub fn create_unregistered_module_info(
        &mut self,
        name: FastString,
        module_type: ModuleType,
        handle: v8::Global<v8::Module>,
        ...
        self.create_module_info_inner(name, module_type, handle, main, requests, false)
    }

    fn create_module_info_inner(
        &mut self,
        name: FastString,
        module_type: ModuleType,
        handle: v8::Global<v8::Module>,
    ...
    pub fn get_id(
        ...
        let first_symbolic_module = map.get(requested_module_type.as_ref(), name)?;
        ...
        loop {
            let symbolic_module = map.get(requested_module_type.as_ref(), mod_name)?;
    ...
    pub(crate) fn alias(
        ...
        self.by_name
            .insert(requested_module_type, name, SymbolicModule::Alias(target));
    }
    ...
    pub fn assert_module_map(&self, modules: &Vec<ModuleInfo>) {
        ...
        for info in modules {
            ...
            let requested_module_type = RequestedModuleType::from(info.module_type.clone());
            assert_eq!(
