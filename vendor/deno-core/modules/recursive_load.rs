...
type ModuleLoadFuture =
    dyn Future<Output = Result<Option<(ModuleRequest, ModuleSource)>, ModuleLoaderError>>;
...
impl RecursiveModuleLoad {
    ...
    /// Override URL lookup with an explicit, unregistered root after the graph
    /// loader has frozen its import-map snapshot. HTML inline modules use this
    /// to keep a distinct ModuleId for each element sharing the document URL.
    pub(crate) fn set_explicit_root(&mut self, root_module_id: ModuleId) {
        self.root_module_id = Some(root_module_id);
    }
    ...
    fn register_and_recurse_inner(&mut self, module_id: usize, module_request: &ModuleRequest) {
