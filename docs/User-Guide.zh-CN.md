...
| `--wait-until` 值 | 返回时机 |
| --- | --- |
| `commit` | URL、实时 parser DOM、V8 realm 与预加载脚本已经安装 |
| `domcontentloaded` | defer/classic/module 队列结束并触发 DOMContentLoaded |
| `load` | 标准 Window load 阻塞集合完成并触发 `window.load`（默认） |
| `networkidle2` | load 后活动请求不超过 2 个并持续 500 ms |
| `networkidle0` | load 后没有活动请求并持续 500 ms |
| `capture-ready` | load 后资源、frame、网络与可观察 DOM 活动安静 500 ms |

`capture-ready` 的默认总预算为 5 秒。timer 本身不阻塞它，但 timer 引起的
DOM 变化或新请求会重新计算 500 ms 静默窗口。它适合截图和归档前观察页面，
但不等同于标准 `window.load`。
...
原始 CDP `Page.navigate` 未指定 `waitUntil` 时会在 document commit 返回，页面
随后继续加载。需要生命周期事件的客户端应先启用 Page domain，并调用
...
这是预期语义：`commit` 是早期边界，流式 parser 可能尚未读完响应。一般使用
默认 `load`；若目标是稳定截图或资源归档，使用 `capture-ready`。
...
- 普通顶层 HTTP document 支持 commit 后流式解析；iframe response 与 stealth
  document transport 当前仍先缓冲；
